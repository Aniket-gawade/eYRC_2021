defmodule ToyRobot.PhoenixSocketClient do
  ...
  def connect_server() do
    ...
    #socket connet using startlink ...wait since it is async...join channel once connected

    #import url from config.exs
    socket_opts = [
      url: Application.fetch_env!(:phoenix_server, :url)
    ]
  ...
  def send_robot_status(channel, %ToyRobot.Position{x: x, y: y, facing: facing} = _robot) do
    ...
    {:ok, is_obs} = PhoenixClient.Channel.push(channel, "new_msg", message)

    {:obstacle_presence, is_obs}
