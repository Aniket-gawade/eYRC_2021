defmodule ToyRobot do
  ...
  def change_orientation_x(robot,error_x) do
    ...
    robot = if(error_x>0) do
      ToyRobot.rotate(robot, :east)

    else
      ToyRobot.rotate(robot, :west)
  ...
  def change_orientation_y(robot,error_y) do
    ...
    robot = if(error_y>0) do
      ToyRobot.rotate(robot, :north)

    else
      ToyRobot.rotate(robot, :south)
  ...
  def go_to_point(robot,goal_x, goal_y,cli_proc_name) do
    ...
    error_y = @robot_map_y_atom_to_num[goal_y]-@robot_map_y_atom_to_num[robot.y]

    robot = if(error_y<0) do
      ToyRobot.change_orientation_y(robot, error_y)
      ToyRobot.move_to(-1*error_y,robot,cli_proc_name)

    else
      ToyRobot.move_to(error_y,robot,cli_proc_name)
    end

    robot = ToyRobot.change_orientation_x(robot, error_x)

    robot = if(error_x<0) do
      ToyRobot.move_to(-1*error_x,robot,cli_proc_name)

    else
      ToyRobot.move_to(error_x,robot,cli_proc_name)
