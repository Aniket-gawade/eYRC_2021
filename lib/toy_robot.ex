defmodule ToyRobot do
  ...
  def move_to(n,robot,channel, obs_list, element_list) when n <= 0 do
    ...
    [robot, obs_list, element_list] = if(node != Enum.at(element_list,length(element_list)-1)) do
      ...
      {:obstacle_presence, is_obs} = ToyRobot.PhoenixSocketClient.send_robot_status(channel, robot)
  ...
  def move_to(n,robot,channel, obs_list, element_list)do
    ...
    [robot, obs_list, element_list] = if(node != Enum.at(element_list,length(element_list)-1)) do
      ...
      [robot, obs_list, element_list] = if(is_obs) do
        ...
        move_to(n-1,robot,channel, obs_list, element_list)
        #break
      else
      robot = move(robot)
      move_to(n-1,robot,channel, obs_list, element_list)
  ...
  def rotate(robot, required, channel) when robot.facing == required do
    # IO.puts(robot.facing)
    robot
  end

  def rotate(robot, required, channel) do
  robot = if(required==@directions_to_the_right[robot.facing]) do
    ...
    rotate(robot, required, channel)
end


def change_orientation_x(robot,error_x, channel) do

  robot = if(error_x>0) do
    rotate(robot, :east, channel)
  else
    robot
  end

  robot = if(error_x<0) do
    rotate(robot, :west, channel)
  else
    robot
  end
...
def change_orientation_y(robot,error_y, channel) do

  robot = if(error_y>0) do
    rotate(robot, :north, channel)
  else
    robot
  end

  robot = if(error_y<0) do
    rotate(robot, :south, channel)
  else
    robot
  end
  ...
  def go_to_point(robot,goal_x, goal_y,channel,obs_list, element_list) do
    ...
    robot = change_orientation_y(robot, error_y, channel)

    if(error_y<0) do
      move_to(-1*error_y,robot,channel, obs_list, element_list)
    else
      move_to(error_y,robot,channel, obs_list, element_list)
    end
  else
    [robot, obs_list, element_list]
  end
  ...
  [robot, obs_list, element_list] = if(error_x != 0) do
    ...
    robot = change_orientation_x(robot, error_x, channel)

    if(error_x<0) do
      move_to(-1*error_x,robot,channel, obs_list, element_list)
    else
      move_to(error_x,robot,channel, obs_list, element_list)
    end
  else
    [robot, obs_list, element_list]
  end
  ...
  def starting_rotate(robot, required, channel) when robot.facing == required do
    # send_robot_status(robot, channel)
    robot
  end

  def starting_rotate(robot, required, channel) do
  robot = if(required==@directions_to_the_right[robot.facing]) do
    ...
    starting_rotate(robot, required, channel)
end


def start_change_orientation_x(robot,error_x, channel) do

  robot = if(error_x>0) do
    starting_rotate(robot, :east, channel)
  else
    robot
  end

  robot = if(error_x<0) do
    starting_rotate(robot, :west, channel)
  else
    robot
  end
...
def start_change_orientation_y(robot,error_y, channel) do

  robot = if(error_y>0) do
    starting_rotate(robot, :north, channel)
  else
    robot
  end

  robot = if(error_y<0) do
    starting_rotate(robot, :south, channel)
  else
    robot
  end
  ...
  def traverse_path_nodes(x, n, robot,channel,path,obs_list, element_list) when x==n do
    [robot,obs_list, element_list]
  end

  def traverse_path_nodes(x, n, robot,channel,path,obs_list, element_list) do
    ...
    robot = if(x==0 and length(element_list)==0) do
      ...
      robot = start_change_orientation_x(robot, error_x, channel)
      robot = start_change_orientation_y(robot, error_y, channel)
    else
      robot
    end
    ...
    [robot,obs_list_new, element_list] = go_to_point(robot,neighbourNode.x,neighbourNode.y,channel,obs_list, element_list)
    ...
    if(length(obs_list_new) != length(obs_list)) do  #break
      traverse_path_nodes(1, 1, robot,channel,path,obs_list_new, element_list)
    else
      traverse_path_nodes(x+1, n, robot,channel,path,obs_list, element_list)
    end
  end


def path_planner(robot ,goal_x,goal_y,channel,obs_list,parent, element_list) when (robot.x==goal_x and robot.y==goal_y) do
  # send_robot_status(robot,channel)
...
def path_planner(robot,goal_x,goal_y,channel,obs_list,parent, element_list) do
  ...
  [robot,obs_list, element_list] = traverse_path_nodes(0, length(path),robot,channel,path,obs_list, element_list)

  path_planner(robot,goal_x,goal_y,channel,obs_list,parent, element_list)
end
  ...
  def stop(robot, goal_x, goal_y, channel) do
    ...
    element_list = []
    parent = self()
    Process.register(spawn_link(fn -> path_planner(robot,goal_x,goal_y,channel,obs_list,parent, element_list) end), :client_toyrobot)
